...
func (v *ClickHouseVisitor) Visit(tree antlr.ParseTree) interface{} {
	...
	case *KeyContext:
		return v.VisitKey(t)
	default:
		// Instead of panicking, return an empty string
		// You might want to log this situation in production code
		return fmt.Sprintf("/* Unsupported node type: %T */", tree)
	}
}
...
func (v *ClickHouseVisitor) VisitComparison(ctx *ComparisonContext) interface{} {
	keyName := v.Visit(ctx.Key()).(*types.QBField)
...
func (v *ClickHouseVisitor) VisitFullText(ctx *FullTextContext) interface{} {
	// In ClickHouse, we might want to search across multiple fields
	// For simplicity, we'll assume there's a fullTextSearch function that handles this
	quotedText := ctx.QUOTED_TEXT().GetText()
	fmt.Println("quotedText", quotedText)
...
func QBFieldFromKeyText(keyText string) *types.QBField {
	keyTextParts := strings.Split(keyText, ".")

	qbField := new(types.QBField)

	// check for the explicit data type
	lastPartParts := strings.Split(keyTextParts[len(keyTextParts)-1], ":")
	if len(lastPartParts) > 1 {
		switch strings.ToLower(lastPartParts[len(lastPartParts)-1]) {
		case "string":
			qbField.DataType = types.QBFieldDataTypeString
		case "number":
			qbField.DataType = types.QBFieldDataTypeNumber
		case "boolean":
			qbField.DataType = types.QBFieldDataTypeBoolean
		case "float64":
			qbField.DataType = types.QBFieldDataTypeNumber
		case "[]string":
			qbField.DataType = types.QBFieldDataTypeStringArray
		case "[]number":
			qbField.DataType = types.QBFieldDataTypeNumberArray
		case "[]boolean":
			qbField.DataType = types.QBFieldDataTypeBooleanArray
		case "[]float64":
			qbField.DataType = types.QBFieldDataTypeNumberArray
		case "[]any":
			qbField.DataType = types.QBFieldDataTypeObjectArray
		default:
			qbField.DataType = types.QBFieldDataTypeUnknown
		}

		// if valid type, remove the data type from last part
		switch qbField.DataType {
		case types.QBFieldDataTypeString, types.QBFieldDataTypeNumber, types.QBFieldDataTypeBoolean, types.QBFieldDataTypeStringArray, types.QBFieldDataTypeNumberArray, types.QBFieldDataTypeBooleanArray, types.QBFieldDataTypeObjectArray:
			lastPartKeyParts := lastPartParts[:len(lastPartParts)-1]
			keyTextParts[len(keyTextParts)-1] = strings.Join(lastPartKeyParts, ":")
		default:
			// unknown type, nothing to do
		}
	}
	// if there is only one part, then no explicit data type provided

	if len(keyTextParts) > 1 {
		switch strings.ToLower(keyTextParts[0]) {
		case "resource":
			qbField.Context = types.QBFieldContextResource
			keyTextParts = keyTextParts[1:]
		case "attribute":
			qbField.Context = types.QBFieldContextAttribute
			keyTextParts = keyTextParts[1:]
		default:
			// no explicit context provided, pass as is
			// check if it's a valid top level field
			if item, ok := types.TraceStaticFieldsTraces[keyText]; ok {
				qbField.Context = types.QBFieldContextPrimary
				qbField.DataType = item.DataType
			} else {
				qbField.Context = types.QBFieldContextUnknown
			}
		}
	} else {
		// now this can either be a top level field
		// or a non-ambiguous attribute
		if item, ok := types.TraceStaticFieldsTraces[keyText]; ok {
			qbField.Context = types.QBFieldContextPrimary
			qbField.DataType = item.DataType
		} else {
	...
	qbField.Key = strings.Join(keyTextParts, ".")
	if qbField.DataType == "" {
		qbField.DataType = types.QBFieldDataTypeUnknown
	}

	// TODO(srikanthccv):
	// now use DB info for the inferred context if
	// not already set
...
func (v *ClickHouseVisitor) VisitKey(ctx *KeyContext) interface{} {
	...
	qbField := QBFieldFromKeyText(keyText)
